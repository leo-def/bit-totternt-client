...
export class HttpTrackerClient implements TrackerClient {
  ...
  async announce(announce: string, request: AnnounceHttpRequest): Promise<AnnounceHttpResponse> {
    ...
    const response = AnnounceHttpResponse.fromResponse(BencodeUtils.handle(responseBuff));
