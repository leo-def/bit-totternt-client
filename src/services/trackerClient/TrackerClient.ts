...
export abstract class TrackerClient {
  ...
  static announce(announce: string, request: AnnounceRequest): Promise<AnnounceResponse> {
    return TrackerClient.getInstance(RequestUtils.getCommunicationProtocol(announce)).announce(announce, request);
  }

  abstract announce(announce: string, request: AnnounceRequest): Promise<AnnounceResponse>;
}
